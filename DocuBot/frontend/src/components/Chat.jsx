  ...
  const handleSendMessage = async () => {
    if (input.length) {
      ...
      try {
        ...
        const botResponse = {
          role: "model",
          text: response.data.result || "No response from model.",
        };
        ...
        chatMessages = [...chatMessages.filter((msg) => msg.role != 'loader'), { role: 'model', text: response.data.result }];
        setMessages(chatMessages);
      } catch (err) {
        ...
        messages.length ? <div className="chat">
          {
            ...
            messages.map((msg, idx) => (
              <div className={msg.role} key={`${msg.role}-${idx}`}>
                {msg.role === "loader" ? (
